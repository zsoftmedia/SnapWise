...
export default function PhotoDialog({ open, onClose, onSave, initial }: Props) {
  const [phase, setPhase] = useState<PhotoItem["phase"]>(initial?.phase ?? "before");
  const [status, setStatus] = useState<PhotoItem["status"]>(initial?.status ?? "not_started");
  const [description, setDescription] = useState(initial?.description ?? "");
  const [employeesOnTask, setEmployeesOnTask] = useState<number>(initial?.employeesOnTask ?? 0);
  const [locationTag, setLocationTag] = useState<string>(initial?.locationTag ?? "");
  const [materialsCsv, setMaterialsCsv] = useState<string>((initial?.materials ?? []).join(", "));

  const handleSave = () => {
    if (!initial) return;
    const materials = materialsCsv.split(",").map((m) => m.trim()).filter(Boolean);
    onSave({
      ...initial,
      phase,
      status,
  ...
  return (
    <SlDialog
      ...
      label="Photo details"
      open={open}
      // Optional: keep dialog from closing via Esc only.
    ...
    >
      ...
      {initial?.dataUrl && (
        <div className="pd-preview">
          ...
          <div className="pd-filemeta">
            <div className="pd-name" title={initial.fileName}>{initial.fileName}</div>
      ...
      <div className="pd-grid">
        ...
        <div className="pd-field">
          ...
          {/* NO hoist here — keep popup inside the dialog */}
          <SlSelect
            value={status}
